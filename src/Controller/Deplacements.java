...
public class Deplacements {
    ...
    public void deplacement(Piece piece, Init_Plateau.Case case_arrivee) { //on appelle cette fonction une fois que l'utilisateur à choisit la pièce à déplacer et la case où il veut la déplacer
        ...
        if (!deplacement_is_possible(piece, case_arrivee.getPosition())) { //une fonction qui permet de définir si le déplacement d'une certaine pièce à un certain endroit est possible ou non (exemple: si on sélectionne une case d'arrivée trop loin -> false // si on veut déplacer une baleine sur la terre -> false)
    ...
    public boolean deplacement_is_possible(Piece piece, Position position_arrivee) {
        if (piece.getAlreadyMovedThisRound()) {
            ...
            String case_type = Init_Plateau.Case.getContent(); //on récupère le type de la case (mer, plage, forêt, montagne)
